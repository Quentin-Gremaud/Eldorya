...
import { RevealFogZoneToAllCommand } from '../commands/reveal-fog-zone-to-all.command.js';
...
describe('RevealFogZoneToAllHandler', () => {
  let handler: RevealFogZoneToAllHandler;
  ...
  beforeEach(() => {
    ...
    mockRepository.load.mockImplementation((_cId: string, playerId: string) =>
      Promise.resolve(createInitializedAggregate(playerId)),
    );

    handler = new RevealFogZoneToAllHandler(
  ...
  it('should load FogState for each player and call revealZone', async () => {
    const command = new RevealFogZoneToAllCommand(
      campaignId, fogZoneId, mapLevelId, 10, 20, 100, 200,
    );
  ...
  it('should save 3 times for 3 players', async () => {
    const command = new RevealFogZoneToAllCommand(
      campaignId, fogZoneId, mapLevelId, 10, 20, 100, 200,
    );
  ...
  it('should skip already-revealed player gracefully', async () => {
    // Player 2 already has this zone revealed
    mockRepository.load.mockImplementation((_cId: string, playerId: string) => {
      const aggregate = createInitializedAggregate(playerId);
  ...
  it('should succeed with 0 reveals for empty campaign', async () => {
  ...
  it('should re-throw non-FogZoneAlreadyRevealed errors', async () => {
