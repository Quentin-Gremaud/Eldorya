...
import { UnequipItemCommand } from '../commands/unequip-item.command.js';
import { Inventory } from '../inventory.aggregate.js';
import type { InventoryRepository } from '../inventory.repository.js';
import type { Clock } from '../../../shared/clock.js';

describe('UnequipItemHandler', () => {
  let handler: UnequipItemHandler;
  ...
  beforeEach(() => {
    ...
    handler = new UnequipItemHandler(mockRepository, mockClock);
  });

  it('should load inventory, call unequipItem, and save', async () => {
    const command = new UnequipItemCommand('char-123', 'item-1', 'user-1');

    await handler.execute(command);

    expect(mockRepository.load).toHaveBeenCalledWith('char-123');
    expect(mockInventory.unequipItem).toHaveBeenCalledWith('item-1', mockClock);
  ...
  it('should propagate error when repository.load() throws', async () => {
    ...
    expect(mockInventory.unequipItem).not.toHaveBeenCalled();
  ...
  it('should propagate domain exception and not save when aggregate method throws', async () => {
    ...
    mockInventory.unequipItem.mockImplementation(() => {
      throw domainError;
    });
