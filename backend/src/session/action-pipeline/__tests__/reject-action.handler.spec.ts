...
import { RejectActionCommand } from '../commands/reject-action.command.js';
...
describe('RejectActionHandler', () => {
  let handler: RejectActionHandler;
  let mockRepository: {
    saveNew: jest.Mock;
    save: jest.Mock;
    load: jest.Mock;
  ...
  beforeEach(() => {
    ...
    handler = new RejectActionHandler(
  ...
  it('should reject action and save aggregate', async () => {
    mockRepository.loadOrCreate.mockResolvedValue(aggregateWithPendingAction());

    const command = new RejectActionCommand(actionId, sessionId, campaignId, gmUserId, 'Too far away');
  ...
  it('should throw SessionNotLiveException when session is not live', async () => {
    ...
    const command = new RejectActionCommand(actionId, sessionId, campaignId, gmUserId, 'reason');
  ...
  it('should throw NotSessionGmException when caller is not GM', async () => {
    ...
    const command = new RejectActionCommand(actionId, sessionId, campaignId, 'other-user', 'reason');
    await expect(handler.execute(command)).rejects.toThrow(NotSessionGmException);
    expect(mockRepository.save).not.toHaveBeenCalled();
  });

  it('should throw FeedbackRequiredException when feedback is empty', async () => {
    mockRepository.loadOrCreate.mockResolvedValue(aggregateWithPendingAction());

    const command = new RejectActionCommand(actionId, sessionId, campaignId, gmUserId, '');
    await expect(handler.execute(command)).rejects.toThrow(FeedbackRequiredException);
    expect(mockRepository.save).not.toHaveBeenCalled();
  });
});
