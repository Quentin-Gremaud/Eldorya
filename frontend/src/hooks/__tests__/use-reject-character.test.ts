...
import { useRejectCharacter } from "../use-reject-character";
...
describe("useRejectCharacter", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockApiFetch.mockResolvedValue(undefined);
  });

  it("should call correct endpoint with POST method and reason body", async () => {
    const { result } = renderHook(
      () => useRejectCharacter(CAMPAIGN_ID),
    ...
    expect(mockApiFetch).toHaveBeenCalledWith(
      "/api/campaigns/campaign-456/characters/char-123/reject",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ reason: "Stats are too high" }),
      }
    );
  });
  ...
  it("should show success toast on rejection", async () => {
    ...
    expect(toast.success).toHaveBeenCalledWith(
      "Character rejected with feedback"
    );
  });
  ...
  it("should show error toast on failure", async () => {
    ...
    expect(toast.error).toHaveBeenCalledWith("Failed to reject character");
  });

  // M10: Optimistic update tests
  it("should optimistically remove rejected character from cache", async () => {
    ...
    await waitFor(() => {
      const cached =
        queryClient.getQueryData<PendingCharacterDetail[]>(QUERY_KEY);
