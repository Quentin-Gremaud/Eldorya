...
import { useValidateAction } from "../use-validate-action";
...
describe("useValidateAction", () => {
  ...
  it("should call API with correct params", async () => {
    const { result } = renderHook(() => useValidateAction(), {
    ...
    expect(mockApiFetch).toHaveBeenCalledWith(
      "/api/campaigns/c1/sessions/s1/actions/a1/validate",
    ...
    expect(body).toEqual({ narrativeNote: "Well done" });
  });
  ...
  it("should rollback cache on API error", async () => {
    ...
    const { result } = renderHook(() => useValidateAction(), { wrapper });
