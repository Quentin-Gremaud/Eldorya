...
import { useRevealFogZone } from "../use-reveal-fog-zone";
...
describe("useRevealFogZone", () => {
  ...
  it("should send POST to correct endpoint with payload", async () => {
    ...
    const { result } = renderHook(() => useRevealFogZone(CAMPAIGN_ID), {
    ...
    await waitFor(() => {
      expect(mockApiFetch).toHaveBeenCalledWith(
        `/api/campaigns/${CAMPAIGN_ID}/fog/reveal`,
  ...
  it("should rollback on error", async () => {
    ...
    await waitFor(() => {
      expect(toast.error).toHaveBeenCalledWith("Failed to reveal fog zone");
    });
  ...
  it("should show success toast on success", async () => {
    ...
    await waitFor(() => {
      expect(toast.success).toHaveBeenCalledWith("Fog zone revealed");
    });
  });
  ...
  it("should trigger delayed invalidation on settled", async () => {
    ...
    // Advance timers by 1.5s for delayed invalidation
    await act(async () => {
